...
function fulfillAvailableOrders({ ordersMetadata, seaportContract, fulfillerBalancesAndApprovals, fulfillerOperator, currentBlockTimestamp, ascendingAmountTimestampBuffer, conduitKey, signer, recipientAddress, exactApproval, domain, overrides, }) {
...
const getAdvancedOrderNumeratorDenominator = (order, unitsToFill) => {
    ...
    if (unitsToFill) {
        ...
        if (unitsToFillBn > maxUnits) {
            throw new Error(`Cannot fill ${unitsToFillBn} units: this order is only divisible into ${maxUnits} unit(s). ` +
