...
class Seaport {
    ...
    config;
    defaultConduitKey;
    OPENSEA_CONDUIT_KEY = constants_1.OPENSEA_CONDUIT_KEY;
    /**
    ...
    ascendingAmountFulfillmentBuffer = 300, balanceAndApprovalChecksOnOrderCreation = true, conduitKeyToConduit, } = {}) {
        ...
        this.contract = index_1.Seaport__factory.connect(seaportContractAddress, this.provider);
        const domainRegistryContractAddress = overrides?.domainRegistryAddress ?? constants_1.DOMAIN_REGISTRY_ADDRESS;
        this.domainRegistry = index_1.DomainRegistry__factory.connect(domainRegistryContractAddress, this.provider);
        this.config = {
    ...
    async createBulkOrders(createOrderInput, accountAddress, exactApproval) {
        ...
        for (const input of createOrderInput) {
            ...
            const seenApprovalKeys = new Set(allApprovalActions.map(approval => (0, approval_1.getApprovalDedupKey)(approval, Boolean(exactApproval))));
            for (const approval of approvalActions) {
                const key = (0, approval_1.getApprovalDedupKey)(approval, Boolean(exactApproval));
     ...
     * @returns a use case containing the set of approval actions and fulfillment action
     */
    async fulfillOrders({ fulfillOrderDetails, accountAddress, conduitKey = this.defaultConduitKey, recipientAddress = ethers_1.ethers.ZeroAddress, domain, exactApproval = false, overrides, }) {
     ...
     * @returns A set of transaction methods (transact, estimateGas, staticCall, buildTransaction).
     */
    matchAdvancedOrders({ orders, criteriaResolvers, fulfillments, recipient, overrides, accountAddress, domain, }) {
        this._validateMatchOrdersNativeValue(orders.map(o => o.parameters), overrides);
        return (0, usecase_1.getTransactionMethods)(this._getSigner(accountAddress), this.contract, "matchAdvancedOrders", [orders, criteriaResolvers, fulfillments, recipient, overrides], domain);
    }
    /**
     ...
     */
    _validateMatchOrdersNativeValue(orderParams, overrides) {
        const hasNativeToken = orderParams.some(params => params.offer.some(item => item.itemType === constants_1.ItemType.NATIVE) ||
            params.consideration.some(item => item.itemType === constants_1.ItemType.NATIVE));
        if (hasNativeToken && !overrides?.value) {
            throw new Error("Orders contain native ETH items but no `value` was provided in overrides. " +
