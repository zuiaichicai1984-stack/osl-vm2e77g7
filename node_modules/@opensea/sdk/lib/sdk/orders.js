...
class OrdersManager {
    ...
    getNFTItems(nfts, quantities = []) {
        return nfts.map((nft, index) => ({
            itemType: (0, utils_1.getAssetItemType)(nft.tokenStandard.toUpperCase()),
     ...
     */
    async _buildListingOrder({ asset, accountAddress, amount, quantity = 1, domain, salt, listingTime, expirationTime, buyerAddress, includeOptionalCreatorFees = false, zone = constants_1.ZERO_ADDRESS, }) {
        ...
        const { executeAllActions } = await this.context.seaport.createOrder({
            ...
            restrictedByZone: zone !== constants_1.ZERO_ADDRESS,
     ...
     */
    async buildListingOrderComponents({ asset, accountAddress, amount, quantity = 1, domain, salt, listingTime, expirationTime, buyerAddress, includeOptionalCreatorFees = false, zone = constants_1.ZERO_ADDRESS, }) {
    ...
    async createOffer({ asset, accountAddress, amount, quantity = 1, domain, salt, expirationTime, zone = (0, utils_1.getSignedZone)(this.context.chain), }) {
        ...
        return this.context.api.postOffer(order, this.context.seaport.contract.target);
    }
    /**
     ...
     * @throws Error if the amount is not greater than 0.
     */
    async createListing({ asset, accountAddress, amount, quantity = 1, domain, salt, listingTime, expirationTime, buyerAddress, includeOptionalCreatorFees = false, zone = constants_1.ZERO_ADDRESS, }) {
        const order = await this._buildListingOrder({
            asset,
            accountAddress,
            amount,
        ...
        return this.context.api.postListing(order, this.context.seaport.contract.target);
    }
    /**
    ...
    async createBulkListings({ listings, accountAddress, continueOnError = false, onProgress, }) {
        ...
        for (const listing of listings) {
            const { asset, amount, quantity = 1, domain, salt, listingTime, expirationTime, buyerAddress, includeOptionalCreatorFees = false, zone = constants_1.ZERO_ADDRESS, } = listing;
            ...
            return this.getPriceParametersCallback(types_1.OrderSide.LISTING, metadata.paymentTokenAddress, amount).then(async ({ basePrice }) => {
                ...
                return {
                    ...
                    salt: metadata.salt
                        ...
                        : undefined,
                    restrictedByZone: metadata.zone !== constants_1.ZERO_ADDRESS,
                    allowPartialFills: true,
                };
            });
        });
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            try {
                const submittedOrder = await this.context.api.postListing(orders[i], this.context.seaport.contract.target);
    ...
    async createBulkOffers({ offers, accountAddress, continueOnError = false, onProgress, }) {
        ...
        for (let i = 0; i < orders.length; i++) {
            ...
            try {
                const submittedOrder = await this.context.api.postOffer(orders[i], this.context.seaport.contract.target);
                submittedOrders.push(submittedOrder);
