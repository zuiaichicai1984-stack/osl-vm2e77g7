...
exports.OpenSeaOAuth = void 0;
exports.extractOpenSeaScopes = extractOpenSeaScopes;
exports.extractWalletAddress = extractWalletAddress;
exports.decodeJwtPayload = decodeJwtPayload;
const api_types_1 = require("@opensea/api-types");
...
class OpenSeaOAuth {
    ...
    async getDiscovery() {
        ...
        const response = await this.fetchWithTimeout(`${this.issuer}/.well-known/openid-configuration`, { headers: { Accept: "application/json" } });
        if (!response.ok) {
        ...
        if (advertisedIssuer !== this.issuer) {
            throw new Error(`OAuth discovery issuer mismatch: expected ${this.issuer}, got ${discovery.issuer}`);
        }
    ...
    async requestDeviceAuthorization(options) {
        ...
        if (!discovery.device_authorization_endpoint) {
            throw new Error("Authorization server does not support the device authorization flow");
        }
        assertSecureUrl(discovery.device_authorization_endpoint, "device_authorization_endpoint");
        const response = await this.fetchWithTimeout(discovery.device_authorization_endpoint, {
            method: "POST",
            headers: { "Content-Type": "application/x-www-form-urlencoded" },
    ...
    async pollDeviceToken(device) {
        ...
        while (Date.now() < deadline) {
            ...
            if (response.ok) {
                return toOAuthToken((await response.json()));
            }
    ...
    async postToken(tokenEndpoint, params) {
    ...
    async fetchWithTimeout(input, init) {
        ...
        try {
            ...
            if (controller.signal.aborted) {
                throw new Error(`OAuth request to ${input} timed out after ${this.timeoutMs}ms`);
            }
            throw error;
        }
        finally {
...
exports.OpenSeaOAuth = OpenSeaOAuth;
/**
...
function assertSecureUrl(value, label) {
    ...
    if (url.protocol !== "https:" && !(url.protocol === "http:" && isLoopback)) {
        throw new Error(`OAuth ${label} must use https (or http on loopback): ${value}`);
    }
}
/**
...
const OPENAPI_AUTH_SCOPE_NAMES = api_types_1.AUTH_SCOPES.map(({ name }) => name);
const OPENAPI_AUTH_SCOPE_NAME_SET = new Set(OPENAPI_AUTH_SCOPE_NAMES);
function buildScopeParam(scopes) {
    if (scopes?.length === 0) {
        throw new Error("OpenSea OAuth scopes cannot be empty; omit scopes to request all default scopes");
    }
    const requestedScopes = [...new Set(scopes ?? OPENAPI_AUTH_SCOPE_NAMES)];
    const unknownScopes = requestedScopes.filter(scope => !OPENAPI_AUTH_SCOPE_NAME_SET.has(scope));
    ...
    const projectRoleScopes = requestedScopes.map(scope => `${ZITADEL_PROJECT_ROLE_SCOPE_PREFIX}${scope}`);
...
function canonicalOpenSeaScopes(scopes) {
 ...
 */
function extractOpenSeaScopes(accessToken) {
    try {
        ...
        if (Array.isArray(claim)) {
            return canonicalOpenSeaScopes(claim.filter((scope) => typeof scope === "string"));
        }
    }
    catch {
...
function tokenOpenSeaScopes(response) {
...
function toOAuthToken(response, previousRefreshToken) {
 ...
 */
function extractWalletAddress(claims) {
