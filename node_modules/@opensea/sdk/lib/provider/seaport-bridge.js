...
exports.createSeaportBridge = createSeaportBridge;
/**
 ...
 */
function createSeaportBridge(params, logger) {
    ...
    if (!rpcUrl) {
        throw new Error("OpenSeaSDK viem entry point requires an HTTP RPC URL. " +
    ...
    if (!explicitRpcUrl && detectedUrl && logger) {
        logger("Seaport bridge: auto-detected RPC URL from viem transport internals. " +
            "Pass `rpcUrl` explicitly for stability across viem versions.");
    }
    const ethersProvider = new ethers_1.JsonRpcProvider(rpcUrl);
    if (!walletClient) {
 ...
 */
class ViemEthersSigner extends ethers_1.AbstractSigner {
    _walletClient;
    ...
    async sendTransaction(tx) {
        const hash = await this._walletClient.sendTransaction({
            ...
            gasPrice: tx.gasPrice != null ? BigInt(tx.gasPrice.toString()) : undefined,
    ...
    async signTransaction() {
        throw new Error("signTransaction is not supported via the viem bridge. " +
...
//# sourceMappingURL=seaport-bridge.js.map